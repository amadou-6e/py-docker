...
VECTOR_SCHEMA_NAME="${VECTOR_SCHEMA_NAME:-public}"
...
# Enable pgvector and initialize compatibility table.
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$VECTOR_DB_NAME" <<-EOSQL
CREATE EXTENSION IF NOT EXISTS vector;
CREATE SCHEMA IF NOT EXISTS ${VECTOR_SCHEMA_NAME};

-- Keep compatibility with manager readiness checks.
